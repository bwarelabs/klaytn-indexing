...
The EVM has a logging functionality used to “write” data outside smart contracts though emitting formatted logs called events. Events allow smart contracts to “print” information on the blockchain in a way that is more searchable and gas efficient than just saving it to its public storage.
...
Example of a sample transaction that called the `_transfer` function:
...
Moreover, there is the Graph Typescript Library which contains utilities for interacting with the entities store and conveniences for handling smart contract data:
...
For Klaytn (EVM-compatible chain) the protocol is `ethereum`, the subgraph project will be created locally using the `subgraph-studio` option, the published source code of the targeted contract is searched on the Ethereum mainnet explorer, therefore the ABI of the contract should be manually supplied instead.
The subgraph slug can be left blank as the subgraph is not meant to be run remotely on The Graph`s hosted service which is not supporting Klaytn.
...
`kind` parameter represents the blockchain type which in this case is EVM-compatible - `ethereum`.
`name` parameter represents the name given to this specific data source to differentiate it from others.
`network` parameter should be equal to the one specified on the indexer node's provider URL.
`address` parameter represents the specific contract whose events are being targeted, it may be intentionally left blank to instruct the subgraph to consider all logs matching on the event signatures alone on any emitting contract on the blockchain.

`abi` specifies the name of the contract within the ABI file.
`startBlock` signals the indexer from which block to start searching for targeted events. It is recommended to use the creation block of the contract if present, otherwise the syncing process will take longer starting from the genesis block without finding any relevant events.
...
Compared to the `Balance` entity, a wallet could have multiple transfers using the same token which does not guarantee uniqueness among the objects of this entity. Instead `id` can be constructed from the block number + log index of the emitted event also accessible inside the handler.

`Account`:
Entity incorporating the list of `Balance` objects and the list of `Transfer` objects referencing the wallet's address used for this entity type as `id`.

&nbsp;

#### Entity Relationships

An entity may have a relationship to one or more other entities in your schema. These relationships are available only at query time. Relationships in The Graph are unidirectional. It is possible to simulate bidirectional relationships by defining a unidirectional relationship on either "end" of the relationship.

Relationships are defined on entities just like any other field except that the type specified is that of another entity.

- One-To-One

The `Balance` entity defines an one-to-one relationship to the `Account` entity at field `account`. This allows a nested GQL query to access the `Account` fields of `account` within an outer query for some `Balance` object(s) which acts as a reverse lookup.

- One-To-Many

The `Account` entity defines two one-to-many relationships to the `Balance` and `Transfer` entities via the `@derivedFrom` directive. However, the list of `Balance` or `Transfer` objects belonging to a specific `Account` is constructed and accessible only at query time, outside subgraph's handlers.
...
Regarding a many-to-many relationship, such as a wallet owning any number of distinct ERC20 tokens and the latter having multiple holder wallets at once, the straightforward, but not scalable, approach would be to model the relationship as derived on one side entity and explicitly stored on the other one.

Therefore, the above schema should be extended by defining a `Token` entity having a list of `Account` holders that should be directly maintained in handlers, while `Account` entity will have its list of owned tokens resolved at query time by finding all `Token` objects including the wallet within their `holders`.
Example
...
Instances of the entities defined in the subgraph's schema will end up being written to the Postgres store. In order to instruct the indexer node how to process each trigger originating from the data source into entities, some form of "handlers" will have to be implemented.
...
Run `graph codegen` command from same directory where `subgraph.yaml` is located:
...
The handler then proceeds to create/update the `Balance` entities associated to both accounts for the ERC20 emitting the log. The subgraph is more general and able to construct balances in all existing tokens on the blockchain, however in our case we filter only logs emitted by `source.address`.<br />

Nonetheless, the subgraph should also insert a new `Transfer` entity to record the unique transfer for later retrieval of a wallet's history by the dApp.

&nbsp;

#### Building the subgraph

The handlers code in `src/mapping.ts` is neither checked nor compiled into WASM at the code generation phase. `graph build` command will handle the compilation and highlight any syntax and typing errors encountered.
Once all the compile errors have been fixed, the subgraph bytecode can be deployed to the indexer:

&nbsp;

#### Create and deploy the built subgraph
...
After the subgraph has been deployed on the indexer node and the indexing starts, the subgraph becomes available in the browser on the explorer page at port `8000` and path `/subgraphs/<subgraph-name>/subgraph/graphql` or for directly querying at path `/subgraphs/<subgraph-name>/subgraph`.
...
This query returns all `Balance` objects associated to the null address (or the first 1000 of them), sorted ascending by their ids along with the nested data of token address and balance value for each one. The query happens by default in the context of the latest indexed block.
...
```
{
  account(id: "0x2968c66f14308673c12812febfa58cfe87c4e5a8", block: {number: 49548688}) {
    balances(first: 1000) {
      token
      amount
    }
...
This query returns the list of balances of the null address in the context of block `10000000`.
